<script lang="ts">
  ...
  onMount(() => {
    ...
    if (!m) {
      estado = 'erro';
      detalhe = 'Aucun code dans ce lien. Demande-en un nouveau.';
      return;
    }
    codigo = m[1];
    ...
    if (!data) {
      estado = 'erro';
      detalhe = 'Code invalide — demande-lui d\'en générer un nouveau.';
      return;
    }
    const standalone =
...
<div class="mt-10 text-center">
  ...
  {:else if estado === 'preview'}
    ...
    <p class="mt-2 text-carvao/70">
      Ce code contient {previa?.ale ?? 0} + {previa?.dea ?? 0} parties terminées. L'importation
      fusionne tout et ne supprime rien.
    </p>
  ...
  {:else if estado === 'fora'}
    <h1 class="text-2xl font-extrabold">Presque là 📲</h1>
    <p class="mx-auto mt-2 max-w-sm text-carvao/70">
      Ce lien s'est ouvert dans ton <b>navigateur</b> — mais ta progression vit dans l'<b>appli Go on!</b>
      sur ton écran d'accueil. Copie le code et colle-le là-bas :
    </p>
    <ol class="mx-auto mt-3 max-w-sm space-y-1 text-left text-sm text-carvao/70">
      <li>1. Appuie sur <b>Copier le code</b> ci-dessous</li>
      <li>2. Ouvre l'appli <b>Go on!</b> sur ton écran d'accueil (sur ordinateur : l'accueil du cours)</li>
      <li>3. Dans <b>Synchroniser</b>, colle-le et importe</li>
    </ol>
