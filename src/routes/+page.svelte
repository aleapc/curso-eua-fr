<script lang="ts">
  ...
  function importar() {
    ...
    msgOk = !!r;
    msg = r
      ? `Importé : +${r.ale} les tiennes, +${r.dea} les siennes. 🎉`
      : 'Code invalide — vérifie que tu l\'as copié en entier (il commence par ES1.).';
  }
...
<section class="mt-2">
  ...
  <p class="mt-1 text-carvao/70">
    Anglais de survie et culture pour les États-Unis : écoute, dis-le à voix haute, et ne te fais pas repérer comme touriste.
  </p>
</section>
...
>
  ...
  <span class="min-w-0 flex-1">
    <span class="block font-bold leading-tight text-carvao">Traducteur hors connexion</span>
    <span class="block text-xs text-carvao/60"
      >Installe <b>Google Traduction</b> et télécharge l'<b>anglais</b> pour l'utiliser hors connexion :
      la caméra lit les menus, étiquettes et formulaires à l'instant.</span
    >
  </span>
  <span class="shrink-0 text-lg text-carvao/30">↗</span>
</a>
<p class="mt-1 px-1 text-[11px] text-carvao/45">
  💡 Configure-le AVANT de prendre l'avion. Il te sauve avec les petites lignes, mais il ne va pas
  commander ta bière à ta place ni te prévenir que la même boisson coûte trois prix différents.
</p>
  ...
  <section class="mt-7">
    ...
    {#if examDoNivel[nivel.nivel] && nivel.episodios.some((e) => e.partes.some((p) => p.pronta))}
      ...
      >
        ...
        <span class="flex-1">
          <span class="block text-sm font-bold">Épreuve de {nivel.nome}</span>
...
<section class="mt-8">
  <div class="card p-4">
    <h2 class="mb-1 font-bold">👯 Synchronise avec ton compagnon de voyage</h2>
    <p class="mb-3 text-sm text-carvao/60">
      Génère un code et envoie-le pour combiner ce que vous avez fait tous les deux : sans compte,
      sans serveur.
    </p>
    ...
    <div class="mt-4 border-t border-black/5 pt-3">
      <p class="mb-2 text-sm text-carvao/60">Tu as un code ? Colle-le ici :</p>
      <textarea
        bind:value={cola}
        placeholder="ES1...."
